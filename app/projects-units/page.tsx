...
export default function ProjectsUnitsPage() {
  ...
  useEffect(() => {
    fetch("https://raf-backend.vercel.app/category/getAllCategoryARForDashboard")
      .then((res) => res.json())
      .then(async (data) => {
        ...
        await Promise.all(
          cats.map(async (cat) => {
            const res = await fetch(`https://raf-backend.vercel.app/unit/getAllUnitByCategoryId/${cat._id}`);
  ...
  const handleUpdate = async (unitId: string, categoryId: string) => {
    ...
    try {
      const res = await fetch(`https://raf-backend.vercel.app/unit/updateunit/${unitId}`, {
        method: "PUT",
        body: formData,
      });
      if (res.ok) {
